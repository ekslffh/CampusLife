...
//		log.info(" {} 로그인 한 이후에 동작하는 핸들러", realUser.getMemName());
		...
		String accType = aw.getAccType();

		if (accType.equals("STD")) {
			// 학생일경우 학생메인페이지로 이동 : /student
			super.setDefaultTargetUrl("/student");
		} else if (accType.equals("PRF")) {
			// 교수일경우 교수메인페이지로 이동 : /professor
			super.setDefaultTargetUrl("/professor");
		} else { // STF
			// 교직원일경우 교직원메인페이지로 이동 : /staff
			super.setDefaultTargetUrl("/staff");
		}
